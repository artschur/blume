...
interface RainbowButtonProps
  extends Omit<React.ButtonHTMLAttributes<HTMLButtonElement>, 'darkbg'> {
  darkbg?: boolean;
}
...
export const RainbowButton = React.forwardRef<
  HTMLButtonElement,
  RainbowButtonProps
>(({ children, className, darkbg, ...props }, ref) => {
  return (
    <button
      ref={ref}
      className={cn(
        ...
        !darkbg ?
          ...
          "text-white border-neutral-800 bg-[linear-gradient(#060606,#060606),linear-gradient(#fff_50%,rgba(255,255,255,0.6)_80%,rgba(0,0,0,0)),linear-gradient(90deg,hsl(var(--color-1)),hsl(var(--color-5)),hsl(var(--color-3)),hsl(var(--color-4)),hsl(var(--color-2)))]",
        className,
      )}
      {...props}
    >
